...
let nomors = m.sender
let anu = await sticker(null, `https://telegra.ph/file/b56704ca114fa80e5ddd2.png`, global.packname, global.author)
  let who
  ...
  if (global.db.data.users[who].nama == 'Zuraa') throw 'Maaf Apakah Kita Saling Kenal?'
  if (global.db.data.users[who].nama == 'Zeltoria') throw 'Lu Siapa Anjirr Mau Rampok Ownerku!!?'
  if (global.db.data.users[who].nama == 'tya') throw 'Ciee mau rampok yaaa.... :v'
  if (global.db.data.users[who].nama == 'Reyn') throw 'Maaf Kak, Orang Yang Kamu Tag Tidak Bisa Di Rampok'
  if (global.db.data.users[who].nama == 'Riyu') return conn.sendFile(m.chat, anu, 'sticker.webp', '', m)
