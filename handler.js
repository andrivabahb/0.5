        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `Limit Kamu Habis, Beli Dengan Cara *${usedPrefix}beli limit 1*`, m)
...
export async function participantsUpdate({ id, participants, action }) {
    ...
    text: {
      ...
      subtitle: `Member: ${groupMetadata.participants.length}`,
      color: `#DDDDDD`,
    },
    avatar: {
      image: pp,
...
let mssg = {
        ...
        if (mssg) return conn.reply(m.chat, mssg, m)
let msg = {
